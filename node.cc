...
Node::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::Node")
    ...
    .AddAttribute ("SystemId", "The systemId of this node: a unique integer used for parallel simulations.",
                   ...
                   MakeUintegerChecker<uint32_t> ())
		.AddAttribute ("NodeType", "This indicates the Node Type. Mainly the Node  configuration; as router or Server. Default configuration is as a router",
									 TypeId::ATTR_GET|TypeId::ATTR_SET,
									 UintegerValue (0),/*0-> Router, 1-> Server*/
									 MakeUintegerAccessor (&Node::m_TypeofNode),
...
void 
Node::SetTypeofNode (uint8_t type)
{
	NS_LOG_FUNCTION (this);
	m_TypeofNode = type;
}
...
Node::DoDispose ()
{
  ...
  #ifndef SoR
  SoREnbl = false;
  SoRAnalyzerWorking = false; 
  srInstance.DoDispose ();
  #endif
...
bool
Node::PromiscReceiveFromDevice (Ptr<NetDevice> device, Ptr<const Packet> packet, uint16_t protocol,const Address &from, const Address &to, NetDevice::PacketType packetType)
{
  NS_LOG_FUNCTION (this << device 
												<< packet 
												<< protocol 
...
bool
Node::NonPromiscReceiveFromDevice (Ptr<NetDevice> device, Ptr<const Packet> packet, uint16_t protocol,const Address &from)
{
	NS_LOG_FUNCTION (this << device 
												<< packet 
												<< protocol 
...
#ifndef SoR
/*This part is added to make enable the basic node as the SoR*/
bool
Node::SoREnable(bool enable)
{
	return SoREnbl = enable;
}

void 
Node::SoRPacketAnalyzer (Ptr<Node> node)
{
	bool result = 0;
	double randValue = 0.0, tempTime = 0.0;
	Time t_reSchedule = Time ();	  
  
  if (!srInstance.IsQueueEmpty ())
  {  
    randValue = m_rng->GetValue (0.0, 1.0);
    // Assume that Service rate is 100Mbps and the Average packet size is 512MB
    // service Mue = servicerate/Average Packet size
    // Random service time
    tempTime = ((-1/(100000000/512*8.0)) *(log (randValue))) * 1000000 ; // microseconds
    t_reSchedule = MicroSeconds (tempTime); 
    result = srInstance.AnalyzePacketInformation(this);	  
    
    m_nextAnalysis = Simulator::Schedule (t_reSchedule, &Node::SoRPacketAnalyzer, this, this);  
  }
  else
  {
    m_nextAnalysis = Simulator::Schedule (MilliSeconds (m_rng->GetValue (5.0, 15.0)),
                                          &Node::SoRPacketAnalyzer, this, this);      
  }
}
#endif
...
{
#ifndef SoR
//	/*This part is added by Janaka Wijekoon for the testing purposes of SoR Functionality*/
//	bool reRoute=0;
	
  if(SoREnbl)
  {	
	  // Copy the packet and pass it to the buffere implemented on SoR module.   
    Ptr<Packet> cp_packet = packet->Copy();
    
    srInstance.addToQueue(cp_packet); //pass the packet to the SoR Queue
    if (!SoRAnalyzerWorking)
    {
      m_nextAnalysis = Simulator::Schedule (MilliSeconds (m_rng->GetValue (2.0, 10.0)), 
	                                            &Node::SoRPacketAnalyzer, this, this);
    }
  }  
	else
	{
		std::cout << "\n\nThis is from Node:" << (int)m_id << " not analyzing " << std::endl;
	}  

//	Ptr<Packet> cp_packet2 = packet->Copy();
//	
//	// pass the packet to the SoR
//  if(SoREnbl)
//  {
//		srInstance.addToQueue(cp_packet); //pass the packet to the SoR Queue
//    reRoute = srInstance.AnalyzePacketInformation(this);
//		
//		// This is the place the rerouting done temporally
//		// This part is necessary to SoR-based redirection 
//		if(reRoute)
//		{
//			srInstance.getModifiedPacket(cp_packet2,this);
//			cp_packet = cp_packet2->Copy();
//		}
//		else
//		{ 
//			cp_packet = packet->Copy();
//		}
//	}
	...
	for (ProtocolHandlerList::iterator i = m_handlers.begin (); i != m_handlers.end (); i++)
	{
					...
					(i->device != 0 && i->device == device))
			{
						...
						i->protocol == protocol)
				{
					...
					{
							i->handler (device, packet, protocol, from, to, packetType); // passes to the routing module
							found = true;
					}
				}
			}

...
public:
  ...
  uint32_t GetSystemId (void) const;
	/**
 		* This will return the Node Type. As the Nodes can be configured
 		* as routers and server, it is mandetory to indicate that what type
 		* of node it is. Therefore added by janaka Wijekoon for the routing purposes.
 		* Here, if the value is 0, the node is configureed as a router,
 		* if the value is configured as 1 the node is configured a server.
 		* */
	uint8_t GetTypeofNode(void) const;
	/** 
 		* set the Type of Node
 		* 0 -> Router
 		* 1 -> Server
 		* */ 
	void SetTypeofNode(uint8_t type);
  /**
  ...
  /**
 	* This Function is added by janaka Wijekoon to enable a node
 	* as an SoR. All packets received to the NetDevice will be forwarded to the SoR class 
	* SoR class then analyze the packets up to Layer 7 information
	* Author: Janaka Wijekoon <janaka@west.sd.ekio.ac.jp>
  */
  bool SoREnable(bool enable);
...
private:
  ...
  /**
 	* \brief for a given service rate, the SoR packet analyzer will be triggered.
	* SoR then analyze the packets up to Layer 7 information
	* Author: Janaka Wijekoon <janaka@west.sd.ekio.ac.jp>
  */  
  void SoRPacketAnalyzer (Ptr<Node> node); 
...
/**
 * Author: Janaka Wijekoon <janaka@west.sd.ekio.ac.jp>
 * 1. To introduce a packet Queue to the Node
 * 2.	To make the regular ns-3 node as an SoR*/

#ifndef SoR
/*For the SoR implementation*/
  bool SoREnbl; //!< enable the SoR functions for this node
  SoR srInstance; //!< the instance of the SoR 
  bool SoRAnalyzerWorking; //!< the analyzer is running 
	EventId m_nextAnalysis; //!< event to schedule the next analysis
